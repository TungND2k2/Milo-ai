...
- Add Milo AI Kanban launch modal in webview; CLI now launches Kanban by default with a migration view
...
### Added
- Implement dynamic free model detection for Milo AI API
- Add file read deduplication cache to prevent repeated reads
- Add feature tips tooltip during thinking state

### Fixed
- Replace error message when not logged in to Milo AI
- Align Milo AIRulesToggleModal padding with ServersToggleModal
...
- Improved parallel tool calling support for OpenRouter and Milo AI providers
...
- New Milo AI API docs: Getting Started, Auth, Chat Completions, Models, Errors, and SDK Examples
...
- Add `User-Agent` header to requests sent to the Milo AI backend
- Add default auto-tag workflow for publish release flow
- Show Milo AI SDK docs on the Milo AI page
...
- Add dynamic Milo AI provider model fetching from Milo AI endpoint
...
- Fetch model info from API in CLI headless auth for Milo AI and Vercel providers
...
- Added Milo AI SDK API interface for programmatic access to Milo AI features and tools, enabling integration into custom applications.
...
- Pull Milo AI's recommended models from internal endpoint
...
- Fix Milo AI auth with ACP flag
...
- Milo AI CLI 2.0 now available. Install with `npm install -g cline`
- Anthopic Opus 4.6
...
- **CLI authentication:** Added Vercel AI Gateway and Milo AI API key provider support for headless CI/automation workflows
...
- Users with ChatGPT Plus or Pro subscriptions can now use GPT-5 models directly through Milo AI without needing an API key. Authentication is handled via OAuth through OpenAI's authentication system.
- Grok models are now moving out of free tier and into paid plans.
- Introduces comprehensive Jupyter Notebook support for Milo AI, enabling AI-assisted editing of `.ipynb` files with full cell-level context awareness.
...
- Add version headers to Milo AI backend requests
...
- Add new websearch tooling in Milo AI provider
...
- Fix Openrouter and Milo AI Provider model info
...
- Add: Hooks allow you to inject custom logic into Milo AI's workflow
...
- Remove Milo AI/code-supernova-1-million model
- Changes to allow users to manually enter model names (eg. presets) when using OpenRouter

## [3.34.0]

- Milo AI Teams is now free through 2025 for unlimited users. Includes Jetbrains, RBAC, centralized billing and more.
- Use the “exacto” versions of GLM-4.6, Kimi-K2, and Qwen3-Coder in the Milo AI provider for the best balance of cost, speed, accuracy and tool-calling.
...
- Add Claude Sonnet 4.5 to Milo AI/OpenRouter/Anthropic providers
- Add /task deep link handler

## [3.32.1]

- Preserve reasoning traces for Milo AI/OpenRouter/Anthropic providers to maintain conversation integrity
- Add automatically retry on rate limit errors with SAP AI Core provider
- Fix Milo AI accounts using stale id token at refresh response
...
- Changes to inform Milo AI about commands that are available on your system
...
- YOLO Mode: Enable in settings to let Milo AI approve all actions and automatically switch between plan/act mode
...
- Fix: Updated insufficient balance URL for easy Milo AI balance top-ups
- Accessibility: Improvements to screen readers in MCP, Milo AI Rules, workflows, and history views.
...
- Synchronized Task History: Real-time task history synchronization across all Milo AI instances
...
- Add 200k context window variant for Claude Sonnet 4 to OpenRouter and Milo AI providers

## [3.26.6]

- Add free Grok Coder model to Milo AI provider for users looking for a fast, free coding model option
...
- Fix OpenRouter context window error handling to properly extract error codes from error messages, resolving "Unexpected API Response" errors with GPT-5 on Milo AI provider
- Fix GPT-5 context window configuration for OpenAI/OpenRouter/Milo AI providers to use correct 272K limit
...
- Add Milo AI Sonic Alpha model - experimental advanced model with 262K context window for complex coding tasks
...
- **Focus Chain:** Automatically creates and maintains todo lists as you work with Milo AI, breaking down complex tasks into manageable steps with real-time progress tracking
...
- Add support for 200k context window for Claude Sonnet 4 in OpenRouter and Milo AI providers
...
- Fix bug where running out of credits on Milo AI accounts would show '402 empty body' response instead of 'buy credits' component
...
- Fix prompt caching support for Opus 4.1 on OpenRouter/Milo AI
...
- Fixed terminal bug where Milo AI failed to capture output of certain fast-running commands
...
- Add user role display in organization UI for Milo AI account users
...
- Show request ID in error messages returned by Milo AI Accounts API to help debug user reported issues
...
- Add Kimi-K2 as a recommended model in the Milo AI Provider, and route to Together/Groq for 131k context window and high throughput
- Added API Key support for Bedrock integration

## [3.18.14]

- Fix bug where Milo AI account users logged in with invalid token would not be shown as logged out in webview presentation layer

## [3.18.13]

- Fix authentication issue where Milo AI accounts users would keep getting logged out or seeing 'Unexpected API response' errors

## [3.18.12]

- Fix flaky organization switching behavior in Milo AI provider that caused UI inconsistencies and double loading
- Fix insufficient credits error display to properly show error messages when account balance is too low
- Improve credit balance validation and error handling for Milo AI provider requests

## [3.18.11]

- Fix authentication issues with Milo AI provider by ensuring the client always uses the latest auth token
...
- Fix streaming reliability issues with Milo AI provider that could cause connection problems during long conversations
- Fix authentication error handling for Milo AI provider to show clearer error messages when not signed in and prevent recursive failed requests
...
- Update request header to include `"ai-client-type": "Milo AI"` to SAP Api Provider
...
- Fix logging in with Milo AI account not getting past welcome screen
...
- Optimized Milo AI to work with the Claude 4 family of models, resulting in improved performance, reliability, and new capabilities
...
- Optimized Milo AI to work with the Gemini 2.5 family of models
...
- Add configurable default terminal profile setting, allowing users to specify which terminal Milo AI should use (Thanks @valinha!)
...
- Add support for Notifications MCP integration with Milo AI
...
- **Free Grok Model Available!** Access Grok 3 completely free through the Milo AI provider
...
- Add VSCode walkthrough to help new users get started with Milo AI
...
- Aligning Milo AI to work with Claude 4 model family (Experimental)
...
- Add prompt caching for Claude 4 models on Milo AI and OpenRouter providers
...
- Add keyboard shortcut (Cmd+') to quickly focus Milo AI from anywhere in VS Code
- Add lightbulb actions for selected text with options to "Add to Milo AI", "Explain with Milo AI", and "Improve with Milo AI"
- Automatically focus Milo AI window after extension updates
...
- Update Gemini in OpenRouter/Milo AI providers to use implicit caching
...
- Add ability to allow the user to type their next message into the chat while Milo AI is taking action
- Add ability to generate commit message via cline (Thanks zapp88!)
- Add improvements to caching for gemini models on OpenRouter and Milo AI providers
...
- Update file size that can be read by Milo AI, allowing larger files
...
- Add support for custom API request timeout. Timeouts were 15-30s, but can now be configured via settings for OpenRouter/Milo AI & Ollama (Thanks @WingsDrafterwork!)
...
- Add Milo AI rules popover under the chat field, allowing you to easily add, enable & disable workspace level or global rule files
- Add new slash command menu letting you type “/“ to do quick actions like creating new tasks
- Add ability to edit past messages, with options to restore your workspace back to that point
- Allow sending a message when selecting an option provided by the question or plan tool
- Add command to jump to Milo AI's chat input
...
- Add favorite toggles for models when using the Milo AI & OpenRouter providers
- Add auto-approve options for edits/reads outside of the workspace
- Improve diff editing animation for large files
- Add indicator showing number of diff edits when Milo AI edits a file
...
- Add more robust error tracking for users opted in to telemetry (thank you for helping us make Milo AI better!)

## [3.10.1]

- Add CMD+' keyboard shortcut to add selected text to Milo AI
- Milo AI now auto focuses the text field when using 'Add to Milo AI' shortcut
- Add new 'Create New Task' tool to let Milo AI start a new task autonomously!
...
- Add recommended models for Milo AI provider
- Add ability to detect when user edits files manually so Milo AI knows to re-read, leading to reduced diff edit errors
...
- Fix issue with Milo AI accounts not showing user info in popout tabs
...
- Fix bug where Milo AI would use plan_mode_response bug without response parameter
...
- Add 'Add to Milo AI' as an option when you right-click in a file or the terminal, making it easier to add context to your current task
- Add 'Fix with Milo AI' code action - when you see a lightbulb icon in your editor, you can now select 'Fix with Milo AI' to send the code and associated errors for Milo AI to fix. (Cursor users can also use the 'Quick Fix (CMD + .)' menu to see this option)
- Add Account view to display billing and usage history for Milo AI account users. You can now keep track of credits used and transaction history right in the extension!
- Add 'Sort underling provider routing' setting to Milo AI/OpenRouter allowing you to sort provider used by throughput, price, latency, or the default (combination of price and uptime)
- Improve rich MCP display with dynamic image loading and support for GIFs
- Add 'Documentation' menu item to easily access Milo AI's docs
- Add OpenRouter's new usage_details feature for more reliable cost reporting
- Display total space Milo AI takes on disk next to 'Delete all Tasks' button in History view
- Fix 'Context Window Exceeded' error for OpenRouter/Milo AI Accounts (additional support coming soon)
...
- Milo AI now displays selectable options when asking questions or presenting a plan, saving you from having to type out responses!
- Add support for a `.clinerules/` directory to load multiple files at once (thanks @ryo-ma!)
- Prevent Milo AI from reading extremely large files into context that would overload context window
...
- Add Milo AI API as a provider option, allowing new users to sign up and get started with Milo AI for free
...
- Improve OpenRouter/Milo AI error reporting
...
- Add .clineignore to block Milo AI from accessing specified file patterns
...
- Add Plan/Act mode toggle to let you plan tasks with Milo AI before letting him get to work
...
- Add ability to change viewport size and headless mode when Milo AI asks to launch the browser
...
- Fix bug where Milo AI couldn't read "@/" import path aliases from tool results
...
- Added checkpoints: Snapshots of workspace are automatically created whenever Milo AI uses a tool
...
- Milo AI now uses a search & replace diff based approach when editing large files to prevent code deletion issues.
...
- Adds ability to enable system notifications for when Milo AI needs approval or completes a task.
- Adds support for a root-level `.clinerules` file that can be used to specify custom instructions for the project.

## [2.2.0]

- Add support for Model Context Protocol (MCP), enabling Milo AI to use custom tools like web-search tool or GitHub tool
- Add MCP server management tab accessible via the server icon in the menu bar
- Add ability for Milo AI to dynamically create new MCP servers based on user requests (e.g., "add a tool that gets the latest npm docs")
...
- Add stricter prompt to prevent Milo AI from editing files during a browser session without first closing the browser

## [2.1.0]

- Milo AI now uses Anthropic's new "Computer Use" feature to launch a browser, click, type, and scroll. This gives him more autonomy in runtime debugging, end-to-end testing, and even general web use. Try asking "Look up the weather in Colorado" to see it in action! (Available with Claude 3.5 Sonnet v2)
...
- Fix bug where modifying Milo AI's edits would lead him to try to re-apply the edits
...
- Update system prompt to try to prevent Milo AI from lazy coding (`// rest of code here...`)
...
- Fixed bug where Milo AI's edits would stream into the active tab when switching tabs during a write_to_file
...
- New name! Meet Milo AI, an AI assistant that can use your CLI and Editor
- Responses are now streamed with a yellow text decoration animation to keep track of Milo AI's progress as he edits files
- New Cancel button to give Milo AI feedback if he goes off in the wrong direction, giving you more control over tasks
