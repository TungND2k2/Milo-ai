...
class ClineEndpoint {
	 ...
	 * Located at ~/.milo/endpoints.json
	 */
	 ...
	 * Priority: bundled endpoints.json → ~/.milo/endpoints.json → null (standard mode)
	...
	public setEnvironment(env: string) {
		if (this.onPremiseConfig) {
			throw new Error("Cannot change environment in on-premise mode. Endpoints are configured via ~/.milo/endpoints.json")
		}
