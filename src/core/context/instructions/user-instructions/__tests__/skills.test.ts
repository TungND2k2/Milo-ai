...
describe("Skills Utility Functions", () => {
	...
	beforeEach(() => {
		...
		sandbox.stub(disk, "getSkillsDirectoriesForScan").returns([
			{ path: path.join(TEST_CWD, ".milorules", "skills"), source: "project" },
...
Instructions here`)
		...
		it("should discover skills from project .milorules/skills directory", async () => {
			const projectSkillsDir = path.join(TEST_CWD, ".milorules", "skills")
...
Use analogies and ASCII diagrams when explaining code.`)
		...
		it("should discover skills from project .milo/skills directory", async () => {
			const miloSkillsDir = path.join(TEST_CWD, ".cline", "skills")
			const skillDir = path.join(miloSkillsDir, "debugging")
			const skillMdPath = path.join(skillDir, "SKILL.md")

			fileExistsStub.withArgs(miloSkillsDir).resolves(true)
			fileExistsStub.withArgs(skillMdPath).resolves(true)
			isDirectoryStub.withArgs(miloSkillsDir).resolves(true)
			readdirStub.withArgs(miloSkillsDir).resolves(["debugging"])
			statStub.withArgs(skillDir).resolves({ isDirectory: () => true })
			readFileStub.withArgs(skillMdPath, "utf-8").resolves(`---
name: debugging
