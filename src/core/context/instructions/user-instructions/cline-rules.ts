...
export const getGlobalClineRules = async (
	...
	if (await fileExistsAtPath(globalClineRulesFilePath)) {
		if (await isDirectory(globalClineRulesFilePath)) {
			try {
				...
				Logger.error(`Failed to read .milorules directory at ${globalClineRulesFilePath}`)
			}
		} else {
	...
	return {
		instructions: formatResponse.miloRulesGlobalDirectoryInstructions(globalClineRulesFilePath, combinedContent),
...
export const getLocalClineRules = async (
	...
	const miloRulesFilePath = path.resolve(cwd, GlobalFileNames.miloRules)

	let instructions: string | undefined
	const activatedConditionalRules: ActivatedConditionalRule[] = []

	if (await fileExistsAtPath(miloRulesFilePath)) {
		if (await isDirectory(miloRulesFilePath)) {
			try {
				const rulesFilePaths = await readDirectory(miloRulesFilePath, [
					[".milorules", "workflows"],
					[".milorules", "hooks"],
					[".milorules", "skills"],
				])
				...
				if (rulesFilesTotal.content) {
					instructions = formatResponse.miloRulesLocalDirectoryInstructions(cwd, rulesFilesTotal.content)
					activatedConditionalRules.push(...rulesFilesTotal.activatedConditionalRules)
				}
			} catch {
				Logger.error(`Failed to read .milorules directory at ${miloRulesFilePath}`)
			}
		} else {
			try {
				if (miloRulesFilePath in toggles && toggles[miloRulesFilePath] !== false) {
					const raw = (await fs.readFile(miloRulesFilePath, "utf8")).trim()
					if (raw) {
						// Keep single-file .milorules behavior consistent with directory/remote rules:
						...
						if (parsed.hadFrontmatter && parsed.parseError) {
							...
							instructions = formatResponse.miloRulesLocalFileInstructions(cwd, raw)
						} else {
							...
							if (passed) {
								instructions = formatResponse.miloRulesLocalFileInstructions(cwd, parsed.body.trim())
								if (parsed.hadFrontmatter && Object.keys(matchedConditions).length > 0) {
									activatedConditionalRules.push({
										name: `${RULE_SOURCE_PREFIX.workspace}:${GlobalFileNames.miloRules}`,
										matchedConditions,
									})
								}
							}
				...
				Logger.error(`Failed to read .milorules file at ${miloRulesFilePath}`)
			}
		}
	}
...
export async function refreshClineRulesToggles(
	...
	const localClineRulesFilePath = path.resolve(workingDirectory, GlobalFileNames.miloRules)
