...
export async function refreshHooks(
	...
	const globalHooksDir = globalHooksDirOverride || path.join(os.homedir(), "Documents", "Milo AI", "Hooks")
	...
	for (const workspacePath of workspacePaths.paths) {
		const workspaceHooksDir = path.join(workspacePath, ".milorules", "hooks")
