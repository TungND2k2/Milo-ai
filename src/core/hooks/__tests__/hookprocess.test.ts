...
describe("HookProcess", () => {
	...
	it("uses resolved PowerShell executable and expected Windows launch args", async () => {
		...
		await withPlatform("win32", async () => {
			const config = await getHookLaunchConfig(
				"C:\\workspace\\.milorules\\hooks\\PreToolUse.ps1",
	...
	it("keeps Unix launch behavior unchanged", async () => {
		await withPlatform("linux", async () => {
			const config = await getHookLaunchConfig("/tmp/.milorules/hooks/PreToolUse")
			config.args.should.deepEqual([])
	...
	it("surfaces resolver failures", async () => {
		await withPlatform("win32", async () => {
			try {
				await getHookLaunchConfig("C:\\workspace\\.milorules\\hooks\\PreToolUse.ps1", async () => {
	...
	it("uses PowerShell on Windows", async () => {
		await withPlatform("win32", async () => {
			const ps1Path = "C:\\workspace\\.milorules\\hooks\\PreToolUse.ps1"
			const resolvedExecutable = "C:\\Program Files\\PowerShell\\7\\pwsh.exe"
	...
	it("coalesces concurrent Windows launcher resolution into a single in-flight resolver call", async () => {
		await withPlatform("win32", async () => {
			...
			const launchRequests = [
				getHookLaunchConfig("C:\\workspace\\.milorules\\hooks\\PreToolUse.ps1", resolver),
				getHookLaunchConfig("C:\\workspace\\.milorules\\hooks\\PostToolUse.ps1", resolver),
				getHookLaunchConfig("C:\\workspace\\.milorules\\hooks\\TaskResume.ps1", resolver),
			]
	...
	it("clears failed launcher cache so later calls can recover", async () => {
		await withPlatform("win32", async () => {
			...
			try {
				await getHookLaunchConfig("C:\\workspace\\.milorules\\hooks\\PreToolUse.ps1", flakyResolver)
				throw new Error("Expected first call to fail")
			} catch (error: any) {
				error.message.should.match(/initial resolver failure/)
			}

			const recoveredConfig = await getHookLaunchConfig("C:\\workspace\\.milorules\\hooks\\PreToolUse.ps1", flakyResolver)
	...
	it("refreshes cached Windows launcher resolution after cache TTL expires", async () => {
		await withPlatform("win32", async () => {
			...
			try {
				const firstConfig = await getHookLaunchConfig("C:\\workspace\\.milorules\\hooks\\PreToolUse.ps1", async () => {
					resolverCallCount += 1
					return resolvedExecutables.shift() || "unexpected"
				})

				const secondConfig = await getHookLaunchConfig("C:\\workspace\\.milorules\\hooks\\TaskResume.ps1", async () => {
					resolverCallCount += 1
					return resolvedExecutables.shift() || "unexpected"
				})
