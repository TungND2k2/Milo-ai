 ...
 * and workspace-specific hooks (from each workspace's .milorules/hooks/) exist for the
 * same hook type.
 *
 * Behavior:
...
function isExpectedHookError(error: unknown): boolean {
	...
	// Note: This is expected because users may have hooks in .milorules that they don't want to execute
...
export class HookFactory {
	 ...
	 * - Workspace hooks (from workspaceRoot/.milorules/hooks/): run from that specific workspace root
	 *
	...
	private determineHookCwd(
		...
		// Workspace hooks are at: workspaceRoot/.milorules/hooks/
	 ...
	 * Workspace hooks are located in workspace .milorules/hooks/ directories
	 *
	 ...
	 * (from .milorules/hooks/ in each workspace root).
	 */
	...
	/**
	 * Finds the path to a hook in a .milorules hooks directory.
	 *
	 * @param hookName the name of the hook to search for, for example 'PreToolUse'
	 * @param hooksDir the .milorules directory path to search
