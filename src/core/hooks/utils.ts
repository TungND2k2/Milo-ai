...
export async function resolveHooksDirectory(
	...
	if (isGlobal) {
		return globalHooksDirOverride || path.join(os.homedir(), "Documents", "Milo AI", "Hooks")
	}
	...
	if (workspaceName) {
		...
		return path.join(targetWorkspace, ".milorules", "hooks")
	}

	// Single workspace: use getCwd
	const cwd = await getCwd(getDesktopDir())
	return path.join(cwd, ".milorules", "hooks")
}
