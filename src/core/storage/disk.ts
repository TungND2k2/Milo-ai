...
export const GlobalFileNames = {
	...
	miloRules: ".milorules",
	workflows: ".milorules/workflows",
	hooksDir: ".milorules/hooks",
	miloruleSkillsDir: ".milorules/skills",
	miloSkillsDir: ".milo/skills",
...
export function getClineHomePath(): string {
	return path.join(os.homedir(), ".milo")
}
...
export async function ensureRulesDirectoryExists(): Promise<string> {
	...
	const miloRulesDir = path.join(userDocumentsPath, "Milo AI", "Rules")
	try {
		await fs.mkdir(miloRulesDir, { recursive: true })
	} catch (_error) {
		return path.join(os.homedir(), "Documents", "Milo AI", "Rules") // in case creating a directory in documents fails for whatever reason (e.g. permissions) - this is fine because we will fail gracefully with a path that does not exist
	}
	return miloRulesDir
}
...
export async function ensureWorkflowsDirectoryExists(): Promise<string> {
	const userDocumentsPath = await getDocumentsPath()
	const clineWorkflowsDir = path.join(userDocumentsPath, "Milo AI", "Workflows")
	try {
		await fs.mkdir(clineWorkflowsDir, { recursive: true })
	} catch (_error) {
		return path.join(os.homedir(), "Documents", "Milo AI", "Workflows") // in case creating a directory in documents fails for whatever reason (e.g. permissions) - this is fine because we will fail gracefully with a path that does not exist
	}
...
export async function ensureMcpServersDirectoryExists(): Promise<string> {
	const userDocumentsPath = await getDocumentsPath()
	const mcpServersDir = path.join(userDocumentsPath, "Milo AI", "MCP")
	try {
		await fs.mkdir(mcpServersDir, { recursive: true })
	} catch (_error) {
		return path.join(os.homedir(), "Documents", "Milo AI", "MCP") // in case creating a directory in documents fails for whatever reason (e.g. permissions) - this is fine since this path is only ever used in the system prompt
	}
...
export async function ensureHooksDirectoryExists(): Promise<string> {
	const userDocumentsPath = await getDocumentsPath()
	const clineHooksDir = path.join(userDocumentsPath, "Milo AI", "Hooks")
	try {
		await fs.mkdir(clineHooksDir, { recursive: true })
	} catch (_error) {
		return path.join(os.homedir(), "Documents", "Milo AI", "Hooks") // in case creating a directory in documents fails for whatever reason (e.g. permissions) - this is fine because we will fail gracefully with a path that does not exist
	}
	return clineHooksDir
}

/**
 * Returns the global skills directory path (~/.milo/skills) without creating it.
 */
...
export function getSkillsDirectoriesForScan(cwd: string): SkillsScanDirectory[] {
	return [
		{ path: path.join(cwd, GlobalFileNames.miloruleSkillsDir), source: "project" },
		{ path: path.join(cwd, GlobalFileNames.miloSkillsDir), source: "project" },
 ...
 * 3. Each workspace root's .milorules/hooks directory (if they exist)
 *
...
/**
 * Gets the paths to the workspace's .milorules/hooks directories to search for
	...
	return (
		await Promise.all(
			workspaceRootPaths.map(async (workspaceRootPath) => {
				// Look for a .milorules/hooks folder in this workspace root.
