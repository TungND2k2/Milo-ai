...
describe("notifications", () => {
	...
	it("passes title and combined subtitle/message to notify-send on Linux", async () => {
		...
		await notificationsModule.showSystemNotification({ title: "Milo AI", subtitle: "Approval Required", message: "test" })

		sinon.assert.calledOnce(execaStub)
		expect(execaStub.firstCall.args[0]).to.equal("notify-send")
		expect(execaStub.firstCall.args[1]).to.deep.equal(["Milo AI", "Approval Required\ntest"])
	})
